...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div>
        ...
        <p class="text-secondary mb-0">Cadastro, edição e inativação sem excluir o histórico do banco de dados.</p>
    </div>
...
<div class="card border-0 shadow-sm mb-4 d-none" id="cardFormulario">
    <div class="card-body">
        ...
        <form id="formPessoa">
            <input type="hidden" name="id" id="pessoald">
            <div class="row g-3">
                <div class="col-md-6">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody id="tabelaPessoas">
                <tr>
                    <td colspan="7" class="text-center py-4">Carregando registos...</td>
                </tr>
            </tbody>
        </table>
    </div>
...
function fecharFormulario() {
    ...
    formPessoa.reset();
    document.getElementById('pessoald').value = '';
}
...
async function carregarPessoas() {
    try {
        const response = await AtendeLabApi.get('pessoas', 'listar');
        const dados = AtendeLabApi.toList(response);
...
async function editarPessoa(id) {
    try {
        const response = await AtendeLabApi.get('pessoas', 'buscarPorId', { id });
        const p = AtendeLabApi.toObject(response);
...
formPessoa.addEventListener('submit', async event => {
    ...
    const id = document.getElementById('pessoald').value;
    try {
...
async function inativarPessoa(id) {
    ...
    try {
        ...
        formData.append('documento', p.documento);
        formData.append('telefone', p.telefone || '');
        formData.append('email', p.email);
        formData.append('curso', p.curso || '');
        formData.append('periodo', p.periodo || '');
        formData.append('status', 'inativo');

        await AtendeLabApi.post('pessoas', 'atualizar', formData);
